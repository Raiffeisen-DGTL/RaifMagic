...
{
    float2 p = position / (size / 2.0);
    
    for (float i = 1.0; i < 7.0; i++)
    {
        p.x += 0.4 / i * cos(i * 2.5 * p.y + time);
        p.y += 0.4 / i * cos(i * 1.5 * p.x + time);
    }
    
    half4 finalColor = half4(half3(61.0/255.0,61/255.0,144.0/255.0)/abs(sin(time-p.y-p.x)), 1.0);
    
    return mix(finalColor, half4(0), (-time) / 3);
}
